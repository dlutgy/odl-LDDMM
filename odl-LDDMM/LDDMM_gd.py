"""
Shape-based registration by Younes large deformation diffeomorphic metric mapping.
"""
...
def LDDMM_gd_solver( template,reference,  time_pts,
                                  niter, eps, lamb, kernel, callback=None):
    """
    Solver for the shape-based registration using LDDMM.

    Notes
    -----
    The model is:
                
        .. math:: \min_{v(t) \in V, \phi_{0,1}^v \in G_v}  \lambda \int_0^1 \|v(t)\|_V^2 dt + \int_\Omega |(\phi_{0,1}^v.I_0) - I_1|^2 |D\phi_{1,0}^v|^{1/2} dx
    ...
    for k in range(niter):
            ...
            for i in range(N+1):
                #first term
                term1_tmp1=-2*(image_N0[i]-image_N1[i])*(detDphi_N0[i])**(1/2)*(detDphi_N1[i])**(1/2)
                term1_tmp = grad_op(image_N0[i])
                #second term
                term2_tmp1=0.5*(detDphi_N0[i])
                term2_tmp=grad_op((image_N0[i]-image_N1[i])**2 *(detDphi_N0[i])**(-1/2) *(detDphi_N1[i])**(1/2))

                for j in range(dim):            
                    term1_tmp[j] *=term1_tmp1
                    term2_tmp[j] *=term2_tmp1
                tmp3 = (2 * np.pi) ** (dim / 2.0) * vectorial_ft_op.inverse(
                    vectorial_ft_op(term1_tmp+term2_tmp) * ft_kernel_fitting)
...
#                    np.exp(inv_N * div_op(vector_fields[i+1])))
                ...
                detDphi_N0[i+1] = (
                    jacobian_det * image_space.element(_linear_deform(
                        detDphi_N0[i], -inv_N * vector_fields[i+1])))
...
#                    np.exp(inv_N * div_o  p(vector_fields[N-i-1])))
            ...
            #print('{!r}'.format(detDphi_N0[1]))

    return image_N0, E,image_N1,detDphi_N0[1],detDphi_N1[1]
